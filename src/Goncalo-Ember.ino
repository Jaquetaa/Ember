// ============================================================
// Goncalo-Ember.ino, Implementação
//
// FICHA TÉCNICA
// -----------------------------------------------------------
// OBJETIVO
// -----------------------------------------------------------
// Ficheiro principal do firmware do drone Ember, executado no
// ESP32-S3 DevKit M1. Coordena o controlo de voo dos quatro
// ESC (Electronic Speed Controller, controlador eletrónico de
// velocidade) via LEDC (LED Controller, controlador PWM (Pulse
// Width Modulation, modulação por largura de pulso) interno do
// ESP32), a receção de comandos de rádio (NRF24L01+ de controlo,
// canal 76), a transmissão de frames térmicos (NRF24L01+ térmico,
// canal 2), a leitura dos sensores de chama e CO (monóxido de
// carbono), e a sequência de calibração dos ESC.
// Cada biblioteca Ember (EmberSensor, EmberDroneNRF, EmberCAMNRF,
// EmberCalibration) é instanciada como global e inicializada em
// setup(). O loop() coordena tudo sem delay(): usa millis()
// (função que devolve os milissegundos (ms) desde que o
// controlador ligou) para temporização não bloqueante.
//
// ORDEM DE EXECUÇÃO
// -----------------------------------------------------------
// 1. setup():
//    Passo 1: inicia a porta série a 115200 bps (bits por
//             segundo, velocidade de transmissão) e aguarda
//             1 s para enumeração USB CDC (Communications
//             Device Class, classe de dispositivo de
//             comunicação USB) do ESP32-S3.
//    Passo 2: configura quatro canais LEDC a 50 Hz (Hz,
//             Hertz, unidade de frequência, ciclos por segundo)
//             e 14 bits, envia THROTTLE_MIN (1000 us) a todos
//             os ESC.
//    Passo 3: força os dois pinos CSN (Chip Select Not,
//             seleciona o dispositivo no bus SPI (Serial
//             Peripheral Interface, protocolo de comunicação
//             série)) a HIGH antes de qualquer comunicação SPI.
//    Passo 4: inicia EmberSensor (sensores de chama e CO).
//    Passo 5: inicia o barramento SPI nos pinos definidos.
//    Passo 6: corre o diagnóstico massivo dos dois módulos NRF.
//    Passo 7: inicia o rádio de controlo RX (Receiver, recetor)
//             via EmberDroneNRF::begin().
//    Passo 8: inicia a câmara térmica TX (Transmitter,
//             transmissor) via EmberCAMNRF::begin().
//    Passo 9: regista o botão de calibração e os callbacks.
// 2. loop():
//    Passo 1: EmberCalibration::update() tem prioridade
//             absoluta. Se isBusy(), drena o rádio para manter
//             o ACK (Acknowledgement, confirmação de receção)
//             activo, descarta comandos de voo e regressa.
//    Passo 2: pré-carrega o ACK com o estado dos sensores e
//             chama controlRX.receive() para ler o payload
//             (conjunto de dados úteis enviados numa mensagem)
//             mais recente.
//    Passo 3: trata EMERGENCY STOP (armed=2) com corte imediato
//             de todos os ESC, antes de qualquer outra lógica.
//    Passo 4: atualiza sensores (leituras ADC (Analog to Digital
//             Converter, conversor analógico-digital) analógicas).
//    Passo 5: imprime estatísticas de diagnóstico a cada 2 s.
//    Passo 6: processa arming/disarming por transições de bordo
//             do campo armed recebido por rádio, com timeout de
//             1 s sem pacotes para emergencyStop() automático.
//    Passo 7: executa o ramp não bloqueante se ativo.
//    Passo 8: se armado, aplica mistura completa de
//             throttle/yaw/pitch/roll nos quatro ESC.
//    Passo 9: EmberCAMNRF::update() captura e transmite o
//             frame térmico em fatias não bloqueantes.
//
// HARDWARE
// -----------------------------------------------------------
// Microcontrolador: ESP32-S3 DevKit M1
// GPIO (General Purpose Input/Output, pino de entrada/saída
//       de uso geral) dos ESC:
//   ESC1: GPIO 14, ESC2: GPIO 4, ESC3: GPIO 1, ESC4: GPIO 19
// NRF controlo RX:  CE=GPIO 13, CSN=GPIO 12
// NRF térmico TX:   CE=GPIO 2,  CSN=GPIO 40
// SPI partilhado:   SCK=GPIO 17, MISO=GPIO 18, MOSI=GPIO 7
// I2C MLX90640:     SDA=GPIO 41, SCL=GPIO 42
// Sensor chama:     GPIO 8 (analógico)
// Sensor CO MQ-7:   GPIO 20 (analógico)
// Botão calibração: GPIO 3 (active LOW, INPUT_PULLUP)
// Botão pré-aquecer: GPIO 39 (active LOW, INPUT_PULLUP)
// ============================================================
...
// -----------------------------------
// Timeout e valores do campo armed
// -----------------------------------

// Tempo em ms sem pacotes de rádio para considerar perda de sinal
...
static inline uint32_t escUsToDuty(uint32_t us) {
  return ((uint64_t)us * ESC_DUTY_MAX) / ESC_PERIOD_US; // uint64_t evita overflow se ESC_RES_BITS for aumentado
}

// -----------------------------------
// Variáveis de estado dos ESC
// -----------------------------------
...
// -----------------------------------
// Diagnóstico massivo do NRF
// -----------------------------------

// Faz 4 testes a niveis cada vez mais baixos:
//   T1: Toggle GPIO, confirma que cada pino consegue HIGH/LOW.
//   T2: Bit-bang SPI, le STATUS register manualmente sem usar
//       o periferico SPI (puro digitalWrite/digitalRead).
//   T3: Hardware SPI raw, usa SPI.transfer() directo.
//   T4: Teste de write/read, escreve em CONFIG e le de volta.
//
// Se T2 (bit-bang) le algo != 0x00, o cablagem esta OK e o
// problema e na lib SPI. Se T3 (hw SPI) le 0xff e T2 le algo
// valido, o periferico SPI nao esta a configurar correctamente.
// Se ambos T2/T3 leem 0x00 ou 0xff, e problema de cablagem
// ou alimentacao.
...
void setup() {
  // Passo 1: inicia série e aguarda enumeração USB CDC
  Serial.begin(115200);
  ...
  // Passo 2: configura os canais LEDC para os quatro ESC
  ...
  // Passo 3: força CSN a HIGH em ambos os NRF antes de iniciar SPI.
  // Se um CSN ficar flutuante durante a init, pode bloquear o bus MISO.
  ...
  // Passo 4: inicia os sensores de chama e CO
  ...
  // Passo 6: diagnóstico massivo antes de tentar a lib RF24
  runNRFDiagnostics();

  // Passo 7: inicia o rádio de controlo (receção de comandos)
  ...
  // Passo 8: inicia a câmara térmica (transmissão de frames)
...
void loop() {
  ...
  // Passo 1: calibração tem prioridade absoluta.
  ...
  // Passo 2: leitura do rádio de controlo.
  ...
  //
  // CRITICO: cache estatico (static) do ultimo comando recebido.
  // O loop corre a >100Hz mas os pacotes chegam a ~50Hz. Sem cache,
  // nas iteracoes sem pacote os ESCs reverteriam aos valores default
  ...
  // Passo 3: EMERGENCY STOP (radio armed=2): corte imediato.
  ...
  // Passo 4: sensores (analog reads, nao bloqueia)
  sensores.update();

  // Passo 5: estatísticas a cada 2 s
  ...
  // Passo 6: lógica de arming/disarming via rádio.
